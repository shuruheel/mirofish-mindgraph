...
# ============== Connect to existing MindGraph graph ==============
...
def connect_mindgraph():
    ...
    MindGraph users have already built a knowledge graph via MindGraph Cloud,
    this API creates a lightweight project to connect directly to the existing graph.

    Request (JSON):
        {
            "simulation_requirement": "optional simulation requirement description",
    ...
    try:
        ...
        # graph_id = project_id, used for namespace isolation when creating new nodes during simulation
        project.graph_id = project.project_id

        ProjectManager.save_project(project)

        logger.info(f"MindGraph connection project created: project_id={project.project_id}")
        ...
        logger.error(f"Failed to create MindGraph connection: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
...
def build_graph():
    ...
    try:
        ...
        errors = []
        if not Config.MINDGRAPH_API_KEY:
            errors.append("MINDGRAPH_API_KEY not configured")
        if errors:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is being built, do not submit again. To force rebuild, add force: true",
        ...
        def build_task():
            ...
            try:
                ...
                # Wait for MindGraph to finish processing
                task_manager.update_task(
                    task_id,
...
def get_graph_data(graph_id: str):
    ...
    Query params:
        source: "upload" (filter by agent_id) or "mindgraph" (read full graph), default "upload"
    """
    try:
        if not Config.MINDGRAPH_API_KEY:
            return jsonify({
