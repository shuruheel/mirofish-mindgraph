...
def generate_report():
    ...
    try:
        ...
        if not simulation_requirement:
            simulation_requirement = "Analyze key findings, important trends, and representative events in group interaction and information propagation during simulation"
...
def get_generate_status():
    ...
    try:
        ...
        # If simulation_id provided, check if completed report exists first
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
# ============== Report Agent conversation APIs ==============
...
def get_report_sections(report_id: str):
    ...
    Frontend can poll this API for generated section content without waiting for entire report
...
def get_agent_log(report_id: str):
    ...
    Query params:
        from_line: Start reading from line N (optional, default 0, for incremental fetching)
...
def get_console_log(report_id: str):
    ...
    Real-time console output during report generation (INFO, WARNING, etc.),
    This differs from the structured JSON log returned by agent-log API,
