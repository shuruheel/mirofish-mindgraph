...
Step2: MindGraph entity reading and filtering, OASIS simulation preparation and execution (fully automated)
"""
...
# Adding this prefix avoids Agent tool calls, making them respond directly with text
...
def get_graph_entities(graph_id: str):
    ...
    try:
        if not Config.MINDGRAPH_API_KEY:
            return jsonify({
                "success": False,
                "error": "MINDGRAPH_API_KEY not configured"
            }), 500
    ...
    try:
        ...
        if not graph_id:
            if project.source == "mindgraph":
                # MindGraph connection mode: use project_id as graph_id (namespace isolation)
        ...
        if active_sims and not clone_from:
            ...
            for sim in active_sims[1:]:
                logger.info(f"Cancelling duplicate simulation: {sim.simulation_id} ({sim.status.value}, "
                           f"profiles={sim.profiles_count}) — keeping {best.simulation_id}")
                sim.status = SimulationStatus.FAILED
                sim.error = f"Superseded by higher-progress simulation {best.simulation_id}"
                manager._save_simulation_state(sim)

            # Return existing best simulation instead of creating a new one
            logger.info(f"Reusing existing simulation: {best.simulation_id} ({best.status.value})")
            return jsonify({
                "success": True,
                "data": best.to_dict()
            })
        ...
        if clone_from:
            ...
            if files_copied:
                logger.info(f"From {clone_from} cloning files to {state.simulation_id}: {files_copied}")
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - preparing: If config_generated=True, it means complete
        # - running: Running, meaning preparation was completed long ago
        # - completed: Run completed, meaning preparation was completed long ago
...
def prepare_simulation():
    ...
    This is a time-consuming operation, the API will immediately return task_id,
    Use GET /api/simulation/prepare/status to check progress
    
    Features:
    - Auto-detect completed preparation, avoid duplicate generation
    - If already prepared, return existing results directly
    - Supports forced regeneration (force_regenerate=true)
    
    Steps:
    1. Check if completed preparation exists
    2. Read and filter entities from MindGraph graph
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Preparation task started | Existing preparation work completed",
    ...
    try:
        ...
        if not force_regenerate and state.status == SimulationStatus.PREPARING:
            ...
            if active_tasks:
                logger.info(f"Simulation {simulation_id} currently preparing (thread alive), skipping duplicate launch")
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
                        "status": "preparing",
                        "message": "Preparation task already in progress",
                        "already_prepared": False
                    }
                })
            else:
                # State says PREPARING but no active task — thread died (e.g. server restart)
                logger.warning(f"Simulation {simulation_id} status is PREPARING but no active tasks, resetting status to allow recovery")
                state.status = SimulationStatus.CREATED
                manager._save_simulation_state(state)

        # Check if already prepared (avoid duplicate generation)
        if not force_regenerate:
            logger.debug(f"Checking if simulation {simulation_id} preparation is complete...")
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        "simulation_id": simulation_id,
                        "status": "ready",
                        "message": "Existing preparation work complete, no need to regenerate",
        ...
        # Get simulation requirement (optional in MindGraph connection mode)
        ...
        if not simulation_requirement:
            simulation_requirement = "Simulate social group interaction and information propagation"

        # Get document text (no documents in MindGraph connection mode)
...
def get_prepare_status():
    ...
    try:
        ...
        # If simulation_id provided, check if preparation is complete first
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Existing preparation work complete",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
            ...
            "data": [
                {
                    ...
                    "simulation_requirement": "If a university announces...",
                    "status": "completed",
                    "entities_count": 68,
                    "profiles_count": 68,
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Real-time Agent Profile retrieval (for viewing progress during generation)
    
    Differences from /profiles API:
    ...
    try:
        ...
        # Check if generating (determined via state.json)
        is_generating = False
        total_expected = None
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Real-time simulation config retrieval (for viewing progress during generation)
    
    Differences from /config API:
    - Reads files directly, bypassing SimulationManager
    - Suitable for real-time viewing during generation
    - Returns additional metadata (e.g. file modification time, whether generating, etc.)
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "enable_graph_memory_update": false,   // Optional: whether to dynamically update Agent activity to MindGraph graph memory
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities during simulation (posts, comments, likes, etc.) are updated to MindGraph graph in real-time
        - This allows the graph to "remember" the simulation process for subsequent analysis or AI conversation
        - Requires simulation-associated project to have a valid graph_id
    ...
    try:
        ...
        max_agents = data.get('max_agents', 27)  # Optional: max Agent count (by graph connectivity, default 27)
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if not process_alive:
                            ...
                            logger.info(f"Simulation {simulation_id} status is RUNNING but process PID={pid} is dead, resetting to READY")
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Enabling graph memory update requires a valid graph_id, please ensure graph is built"
                }), 400
            
            logger.info(f"Enabling graph memory update: simulation_id={simulation_id}, graph_id={graph_id}")
        
        # Get project source (for graph memory update source parameter)
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # Get current round actions (recent_actions shows latest round only)
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (entered wait-for-command mode after completing simulation loop)
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been closed. Please ensure simulation is complete and in wait-for-command mode."
            }), 400
...
def interview_agents_batch():
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            # Validate each item platform (if present)
        ...
        # Optimize each interview item prompt, add prefix to prevent Agent tool calls
    ...
    Request (JSON):
        {
            ...
            "agent_id": 0,                 // Optional, get only this Agent interview history
...
def close_simulation_env():
    ...
    Send close environment command to simulation, making it gracefully exit wait-for-command mode.
    
    Note: This differs from /stop API, /stop forcefully terminates the process,
