...
ReACT-pattern simulation report generation using MindGraph

Features:
1. Generate reports based on simulation requirements and MindGraph graph data
...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Log section content generation complete (records content only, does not indicate entire section is complete)"""
        self.log(
...
[Breadth Search - Get full panoramic view]
...
- Need to obtain simulation Agents' real responses (from OASIS simulation environment)
- Want to make the report more vivid by including "interview records"
...
[Cognitive Analysis - Knowledge graph deep structure query]
Directly query the cognitive structure in the simulation knowledge graph to discover deep patterns:
- weak_claims: Find claims with the lowest confidence (most uncertain predictions)
- contradictions: Find contradictory viewpoints between Agents
- open_questions: Find unanswered questions in the simulation

[Use Cases]
- When writing sections about "disagreements and uncertainties"
- When needing to discover contradictions and controversies between Agents
- When needing to find the most uncertain parts of predictions

[Returns]
- Low confidence claim list (with confidence scores)
- Contradictory viewpoint pairs (with contradiction types)
- Open question list"""

TOOL_DESC_GRAPH_EXPLORE = """\
[Graph Exploration - Reasoning chain tracing and belief evolution]
Deeply explore the knowledge graph structure, tracing reasoning chains and belief evolution:
- chain: Starting from an entity, trace its reasoning chain (causal relationships, support/refutation chains)
- history: View an entity's evolution history during simulation (belief changes, confidence changes)

[Use Cases]
- Need to answer "How was a particular viewpoint formed?"
- Need to show "How did public opinion evolve?"
- Need to track attitude changes of a specific Agent/topic

[Returns]
- Reasoning chain: Logical paths between nodes (support, refutation, citation relationships)
- Evolution history: Version list of nodes (content and confidence changes for each version)"""
...
- Agent behaviors and statements in the simulated world are predictions of future human group behavior
...
into the simulated world. Agent behaviors and interactions in the simulation are predictions of future human group behavior.
...
3. [Language consistency - quoted content must be translated to the report language]
   - Tool-returned content may contain mixed language expressions
   - If the simulation requirement and source materials are in a specific language, the report must be written entirely in that language
...
Social media served as the primary scene for public opinion, taking on the core function of information dissemination:
...
[Data Priority -- Very Important]
The report must be based on events that actually occurred in the simulation, not original document knowledge:
1. First use simulation_search to retrieve simulation Agents' actual behaviors (posts, decisions, interactions)
2. Then use insight_forge for deep context (which also retrieves document knowledge as background)
3. Never present knowledge from original documents as simulation findings

[Tool Usage Recommendations - Please mix different tools]
- simulation_search: [Use first] Search for Agents' actual behaviors and statements during simulation
...
2. Must call tools to retrieve simulation data before writing
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    ...
    "Please call more tools to get additional simulation data before outputting Final Answer. {unused_hint}"
)
...
class ReportAgent:
    ...
    def __init__(
        ...
        Args:
            ...
            source: Project source "upload" or "mindgraph"
        """
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "simulation_search": {
                ...
                "description": "Search data generated during simulation (Agent posts, decisions, anomalies, etc.). "
                    "This is the most important tool -- the report should be primarily based on simulation data, not original document knowledge. "
                    "Returns Agents' actual behaviors and statements from the simulation.",
                "parameters": {
                    "query": "Search query (about what happened in the simulation)",
                    "limit": "Number of results to return (optional, default 15)"
                }
            },
            "insight_forge": {
                "name": "insight_forge",
            ...
            "cognitive_analysis": {
                ...
                "parameters": {
                    "analysis_type": "Analysis type: weak_claims / contradictions / open_questions",
            ...
            "graph_explore": {
                ...
                "parameters": {
                    "mode": "Exploration mode: chain (reasoning chain tracing) / history (belief evolution history)",
                    "entity_name": "Name of entity to explore (e.g., person name, organization name, topic name)",
                    "max_depth": "Maximum reasoning chain depth (optional, default 3, only used in chain mode)"
                }
            }
        }
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "simulation_search":
                ...
                # Cognitive analysis -- weak claims/contradictions/open questions
                ...
                # Graph exploration -- reasoning chain/belief history
                mode = parameters.get("mode", "chain")
                entity_name = parameters.get("entity_name", "")
                if not entity_name:
                    return "Please provide the entity_name parameter"
                if mode == "history":
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        2. Bare JSON (entire response or a single line is a tool call JSON)
        """
        tool_calls = []
    ...
    def _generate_section_react(
        ...
        4. Repeat until sufficient information or max iterations
        ...
        for iteration in range(max_iterations):
            ...
            # ── Conflict handling: LLM output both tool calls and Final Answer ──
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    ...
                    messages.append({
                        "role": "user",
                        "content": (
                            "[Format Error] You included both a tool call and Final Answer in the same reply. This is not allowed.\n"
                    ...
                    # Third time: downgrade, truncate to first tool call and force execute
                    logger.warning(
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Start reading from this line (for incremental fetching, 0 means from beginning)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Because section title is added by the system, content should have no headings
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if heading is duplicate (same content heading within 5 lines)
                is_duplicate = False
