...
CHINA_TIMEZONE_CONFIG = {
    ...
    # Night period (declining activity)
    "night_hours": [23],
...
class TimeSimulationConfig:
    ...
    # Peak hours (evening 19-22, most active time in China)
...
class SimulationConfigGenerator:
    ...
    information, then automatically generates optimal simulation parameter configuration.
    ...
    AGENTS_PER_BATCH = 50
    # Maximum active Agents per round (hard cap to prevent LLM from generating excessively high values that slow simulation)
    ...
    def generate_config(
        ...
        """
        Intelligently generate complete simulation configuration (step-by-step, with checkpoint resume support)
        ...
        Args:
            ...
            checkpoint_dir: Checkpoint directory (simulation directory) for resume support
        ...
        if checkpoint_dir:
            ...
            if os.path.exists(checkpoint_path):
                try:
                    ...
                    logger.info(f"Loaded config checkpoint: completed_step={checkpoint_data.get('completed_step', 0)}, "
                               f"agent_batches={checkpoint_data.get('completed_agent_batches', 0)}")
                except Exception as e:
                    logger.warning(f"Failed to load config checkpoint: {e}")
                    checkpoint_data = {}
        ...
        def save_checkpoint(step: int, data: dict):
            ...
            try:
                ...
                logger.warning(f"Failed to save config checkpoint: {e}")
        ...
        if completed_step >= 1 and "time_config" in checkpoint_data:
            logger.info("[1/...] Restoring time config from checkpoint")
        ...
        if completed_step >= 2 and "event_config" in checkpoint_data:
            logger.info("[2/...] Restoring event config from checkpoint")
        ...
        for batch_idx in range(num_batches):
            ...
            if batch_idx < completed_agent_batches:
                logger.info(f"[{3 + batch_idx}/{total_steps}] Skipping Agent config from checkpoint ({start_idx + 1}-{end_idx}/{len(entities)})")
                continue
    ...
    def _build_context(
        ...
        remaining_length = self.MAX_CONTEXT_LENGTH - current_length - 500  # Reserve 500 character margin
...
- The user group is Chinese, schedule must follow Beijing Time daily habits
- 0-5am: almost no activity (activity coefficient 0.05)
- 6-8am: gradually becoming active (activity coefficient 0.4)
- 9am-6pm: moderate activity during work hours (activity coefficient 0.7)
- 7-10pm: peak period (activity coefficient 1.5)
- After 11pm: declining activity (activity coefficient 0.5)
- General pattern: low activity at dawn, gradual increase in morning, moderate during work hours, evening peak
- **Important**: The example values below are for reference only. You need to adjust specific time periods based on the nature of the event and characteristics of participant groups.
  - Example: Student groups may peak at 9-11pm; media is active all day; official institutions only during work hours
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. Time configuration must follow Chinese daily schedule habits."
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        ...
        # Hard cap: prevent LLM from generating excessively high values causing hundreds of LLM calls per round
        hard_cap = self.MAX_AGENTS_PER_HOUR
        if agents_per_hour_max > hard_cap:
            logger.warning(f"agents_per_hour_max ({agents_per_hour_max}) exceeds hard cap ({hard_cap}), corrected")
...
- **Schedule follows Chinese daily habits**: Almost no activity 0-5am, most active 7-10pm
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<active hour list, following Chinese daily schedule>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configuration must follow Chinese daily schedule habits."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Rule-based generation of a single Agent configuration (Chinese daily schedule)"""
