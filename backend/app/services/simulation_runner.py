...
Runs simulations in background and records each agent's actions, with real-time status monitoring
"""
...
class SimulationRunner:
    ...
    def recover_monitors(cls):
        ...
        for sim_id in os.listdir(cls.RUN_STATE_DIR):
            ...
            try:
                ...
                logger.info(f"Marking dead simulation: {sim_id} (PID={pid})")
                continue
            ...
            if sim_id not in cls._monitor_threads:
                ...
                recovered += 1
                logger.info(f"Recovering monitor thread: {sim_id} (PID={pid})")

        if recovered:
            logger.info(f"Recovered {recovered}  simulation monitor threads")
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Max simulation rounds (optional, for truncating long simulations)
        max_agents: int = 0,  # Max agent count (ranked by graph connectivity, 0=unlimited)
        enable_graph_memory_update: bool = False,  # Whether to update activities to MindGraph graph
        graph_id: str = None,  # MindGraph graph ID (required when graph update is enabled)
        source: str = "upload",  # Project source ("upload" or "mindgraph")
    ) -> SimulationRunState:
        """
        Start simulation
        ...
        Args:
            ...
            enable_graph_memory_update: Whether to dynamically update agent activities to MindGraph graph
            graph_id: MindGraph graph ID (required when graph update is enabled)
        ...
        if existing and existing.runner_status in [RunnerStatus.RUNNING, RunnerStatus.STARTING]:
            ...
            logger.info(f"Simulation {simulation_id} has status {existing.runner_status.value} but process is dead (PID={pid}), allowing restart")
        ...
        if enable_graph_memory_update:
            ...
            try:
                # Load agent node UID mapping (created by simulation_manager.prepare_simulation)
                agent_node_uids = {}
        ...
        try:
            ...
            # Set working directory to simulation directory (database files etc. generated here)
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Inter-round decay + record round observation
                                    if graph_updater:
                            ...
                            # If graph memory update is enabled, send activities to MindGraph
                            if graph_updater:
    ...
    def _read_actions_from_file(
        ...
        Args:
            ...
            default_platform: Default platform (used when action records lack a platform field)
    ...
    def cleanup_all_simulations(cls):
        ...
        if not has_processes and not has_updaters:
            return  # Nothing to clean up, returning silently
                ...
                if process.poll() is None:  # Process still running
                    ...
                    try:
                        ...
                        if os.path.exists(state_file):
                            ...
                            logger.info(f"Updated state.json to stopped status: {simulation_id}")
                        else:
    ...
    def register_cleanup(cls):
        ...
        # In Flask debug mode, only register cleanup in reloader child process (the process actually running the app)
        # WERKZEUG_RUN_MAIN=true indicates the reloader child process
        # In non-debug mode, this env var is absent, so registration is also needed
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None: Interview on both platforms simultaneously in dual-platform mode, return integrated results
    ...
    def close_simulation_env(
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Close environment command sent (response timeout, environment may be closing)"
            }
    ...
    def _get_interview_history_from_db(
        ...
        try:
            ...
            logger.error(f"Read Interview history failed ({platform_name}): {e}")
    ...
    def get_interview_history(
        ...
        Args:
            ...
            agent_id: Specified Agent ID (optional, get only this agent's history)
        ...
        if platform in ("reddit", "twitter"):
            ...
            # When no platform specified, query both platforms
