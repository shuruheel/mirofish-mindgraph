...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from the database, enriched with full context information.

    Args:
        db_path: Database file path
        last_rowid: Last read max rowid value (using rowid instead of created_at because different platforms use different created_at formats)
...
def _enrich_action_context(
    ...
    try:
        ...
        # Repost: enrich with original post content and author
            ...
            if new_post_id:
                # Reposted post's original_post_id points to the original post
                cursor.execute("""
        ...
        # Follow user: enrich with followed user's name
        ...
        # Mute user: enrich with muted user's name
        ...
        # Create comment: enrich with commented post's information
        ...
        # Enrichment failure doesn't affect the main flow
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    for different platforms, increasing concurrency capacity.
    ...
    # Select which LLM to use based on parameters and available config
...
def get_active_agents_for_round(
    ...
    """Determine which Agents to activate this round based on time and config (supports graph relationship boost)"""
...
async def run_twitter_simulation(
    ...
    if _graph_provider is None:
        ...
        if provider:
            log_info(f"Graph context enabled: {len(provider.get_relationship_map())} relationships")
    ...
    # Note: do not close the environment, keep it available for Interview use
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM config (if available, otherwise falls back to standard config)
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Max simulation rounds (optional, for truncating overly long simulations)'
    )
    parser.add_argument(
        '--max-agents',
        type=int,
        default=0,
        help='Max Agent count (sorted by graph connectivity, top N; 0=unlimited)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
    ...
    agent_configs = config.get('agent_configs', [])
    log_manager.info(f"  - Agent count: {len(agent_configs)}")
    ...
    if args.max_agents > 0 and len(agent_configs) > args.max_agents:
        try:
            ...
            log_manager.info(
                f"  - Agent filtering: {len(agent_configs) + len(removed)} -> {len(agent_configs)} "
                f"(by graph connectivity, edge count range: {min_e}-{max_e})"
            )
        except Exception as e:
            log_manager.info(f"  - Agent filtering failed (using all): {e}")
