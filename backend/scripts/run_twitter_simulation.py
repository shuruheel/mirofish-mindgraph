...
This script reads parameters from a config file to run the simulation fully automatically
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        Uses config from project root .env file (highest priority):
        - LLM_API_KEY: API key
...
async def main():
    ...
    # Initialize log config (use fixed file names, clean up old logs)
...
def setup_signal_handlers():
    ...
    Give the program a chance to cleanly release resources (close database, environment, etc.)
    """
