    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content is still being processed. Consider refreshing the graph manually later.</span>
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    // Calculate curvature: spread out when multiple edges, straight line for single edge
    let curvature = 0
    if (totalCount > 1) {
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravity toward center, keep isolated node clusters near the center
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset from line direction, proportional to distance for visible curves
  ...
  // Link labels background (white bg for better text readability)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only record position, don't restart simulation (differentiate click vs drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect if truly dragging (moved beyond threshold)
      ...
      .on('end', (event, d) => {
        // Only decelerate simulation if truly dragged
        if (d._isDragging) {
