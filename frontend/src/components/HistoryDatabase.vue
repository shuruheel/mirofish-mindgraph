        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 "Run Simulation" and Step 5 "Deep Interaction" must be launched live and do not support history replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
// Card layout config - wider proportions
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate card count in current row, ensure row is centered
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Use debounce delay for state transitions, prevent rapid flickering
...
.grid-pattern {
  ...
  /* Position from top-left, height changes only expand at bottom, not affecting existing grid positions */
