...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics
    const factMatch = text.match(/Relevant prediction facts:\s*(\d+)/i)
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeMatch = text.match(/Currently active facts:\s*(\d+)/i)
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Example: - Selected Parent_601(index 0): As a parent group representative...
        if (!headerMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If the first question is preceded by "1.", handle it specially
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/Found\s*(\d+)\s*relevant/i)
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle buttons (only shown when both have real answers)
...
// Currently active step (shown at top)
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after Final Answer: (alternate format)
